...
bool ApiClient::sendChatMessage(const QString &message,
                                const QString &scope,
                                ...
                                QJsonObject *responseObject)
{
    ...
    if (pdfFilePath.trimmed().isEmpty()) {
        ...
        if (conversationId > 0) {
            payload.insert("conversation_id", static_cast<double>(conversationId));
        }

        bool ok = false;
        QJsonDocument doc = post("/chat/message", payload, &ok, errorMessage);
    ...
    if (!fileInfo.exists() || !fileInfo.isFile()) {
        if (errorMessage) {
            *errorMessage = tr("El archivo PDF no existe.");
        }
        return false;
    }

    if (fileInfo.suffix().toLower() != QStringLiteral("pdf")) {
        if (errorMessage) {
            *errorMessage = tr("El chat solo acepta archivos PDF.");
        }
        return false;
    }

    QFile *file = new QFile(pdfFilePath);
    if (!file->open(QIODevice::ReadOnly)) {
        if (errorMessage) {
            *errorMessage = tr("No se pudo abrir el PDF: %1").arg(file->errorString());
        }
        delete file;
        return false;
    }
    ...
    messagePart.setHeader(QNetworkRequest::ContentDispositionHeader,
                          QVariant(QStringLiteral("form-data; name=\"message\"")));
    messagePart.setBody(message.toUtf8());
    multiPart->append(messagePart);

    QHttpPart scopePart;
    scopePart.setHeader(QNetworkRequest::ContentDispositionHeader,
                        QVariant(QStringLiteral("form-data; name=\"scope\"")));
    scopePart.setBody(scope.toUtf8());
    multiPart->append(scopePart);

    if (conversationId > 0) {
        QHttpPart conversationPart;
        conversationPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                                   QVariant(QStringLiteral("form-data; name=\"conversation_id\"")));
        conversationPart.setBody(QString::number(conversationId).toUtf8());
        multiPart->append(conversationPart);
    }
    ...
    contextPart.setBody(QJsonDocument(context).toJson(QJsonDocument::Compact));
    multiPart->append(contextPart);

    QHttpPart filePart;
    filePart.setHeader(QNetworkRequest::ContentDispositionHeader,
                       QVariant(QString("form-data; name=\"archivo\"; filename=\"%1\"").arg(fileInfo.fileName())));
    filePart.setHeader(QNetworkRequest::ContentTypeHeader, QVariant(QStringLiteral("application/pdf")));
    filePart.setBodyDevice(file);
    file->setParent(multiPart);
    multiPart->append(filePart);

    QUrl url(m_apiBaseUrl + "/chat/message-with-file");
    QNetworkRequest request(url);
    request.setRawHeader("Authorization", basicAuthValue());
    request.setRawHeader("Accept", "application/json");
    ...
    if (!timeout.isActive()) {
        ...
        reply->deleteLater();
        if (errorMessage) {
            *errorMessage = tr("Tiempo de espera agotado enviando PDF al chat.");
        }
        return false;
    }
    ...
    if (!doc.isObject()) {
        if (errorMessage) {
            *errorMessage = tr("La respuesta del chat no es un objeto JSON válido.");
        }
        reply->deleteLater();
        return false;
    }
